...
export default function App() {
  ...
  // Listen for video selection coming back from PlaylistPage
  useEffect(() => {
    if (location.state?.selectedVideo) {
      ...
      setVideoData({
        ...
        embedId: video.id,
        isLive: false,
        thumbnail: "churchImages/JesusIsBetter.png"
      });
      document.getElementById('video-player-section')?.scrollIntoView({ behavior: 'smooth' });
    }
  }, [location.state]);

  // Fetch all playlists automatically on mount
  useEffect(() => {
  ...
  // Navigates to the dynamic playlist route
                ...
                videoData && (
                  <div className="max-w-5xl mx-auto bg-white p-3 sm:p-4 rounded-xl shadow-lg border border-gray-100 mb-12 text-left">
                    ...
                    {videoData.isM3u8 ? (
                      ...
                      <FadeYouTubePlayer 
                        embedId={videoData.embedId} 
                        title={videoData.title} 
                        overlayImg={videoData.thumbnail} 
                      />
                    ) : (
                      <div className="relative w-full aspect-video rounded-lg overflow-hidden shadow-inner bg-black flex flex-col items-center justify-center p-4 text-center">
...
function FadeYouTubePlayer({ embedId, title, overlayImg }) {
  ...
  const handlePlay = () => {
    ...
    const iframe = document.getElementById(`yt-player-${embedId}`);
  ...
  return (
    <div className="relative w-full aspect-video rounded-lg overflow-hidden shadow-inner bg-black">
      <iframe
        id={`yt-player-${embedId}`}
        className="absolute top-0 left-0 w-full h-full"
        src={`https://www.youtube.com/embed/${embedId}?enablejsapi=1&autoplay=1`}
        title={title}
        frameBorder="0"
